...
HTTP_SESSION_HANDLE  HTTPClientOpenRequest (HTTP_CLIENT_SESSION_FLAGS Flags)
{
    ...
    OAL_U32         nAllocationSize;             // Size of the dynamically allocated buffer
    
    // Attempt to allocate the buffer
    //printf("Session Ptr lives at %p.\n", (void*)&pHTTPSession);

    pHTTPSession = (P_HTTP_SESSION)OAL_malloc(ALIGN(sizeof(HTTP_SESSION)));
    ...
    if(HTTP_CLIENT_MEMORY_RESIZABLE)
    {
        ...
        nAllocationSize = MINIMUM(HTTP_CLIENT_MAX_SEND_RECV_HEADERS,HTTP_CLIENT_INIT_SEND_RECV_HEADERS);
    ...
    pHTTPSession->HttpHeaders.HeadersBuffer.pParam = (OAL_S8*)OAL_malloc(ALIGN(nAllocationSize));
    ...
    HTTPClientSetVerb((OAL_U32)pHTTPSession,(HTTP_VERB)HTTP_CLIENT_DEFAULT_VERB);    // Default HTTP verb
    ...
    // Return an allocated session pointer (cast to OAL_U32 first)
...
OAL_U32  HTTPClientReset (HTTP_SESSION_HANDLE pSession)
{
...
OAL_U32 HTTPClientCloseRequest (HTTP_SESSION_HANDLE *pSession)
{
    ...
    if(pHTTPSession->HttpHeaders.HeadersBuffer.pParam)
    {
        ...
        OAL_free(pHTTPSession->HttpHeaders.HeadersBuffer.pParam);
    }
...
OAL_U32  HTTPClientSetDebugHook (HTTP_SESSION_HANDLE pSession,
...
OAL_U32  HTTPClientSetVerb (HTTP_SESSION_HANDLE pSession,
                           HTTP_VERB HttpVerb)  // [IN] Http verb (GET POST HEAD act')
{
    ...
    if(pHTTPSession->pDebug)
    {
        pHTTPSession->pDebug("HTTPClientSetVerb",NULL,0,"Selected Verb is %d",(OAL_S32)HttpVerb);
    }
#endif
...
OAL_U32  HTTPClientSetAuth (HTTP_SESSION_HANDLE pSession,
                           ...
                           void *pReserved)
{
    ...
    if(pHTTPSession->pDebug)
    {
        pHTTPSession->pDebug("HTTPClientSetAuth",NULL,0,"Selected authentication is %d",(OAL_S32)AuthSchema);
    }
#endif
...
OAL_U32 HTTPClientSetProxy (HTTP_SESSION_HANDLE pSession,
                           OAL_S8 *pProxyHost,        // [IN] Null terminated string containing the host name
                           OAL_U16 nPort,            // [IN] The proxy port number
                           OAL_S8 *pUserName,         // [IN] User name for proxy authentication (can be null)
                           OAL_S8 *pPassword)         // [IN] User password for proxy authentication (can be null)
{
    P_HTTP_SESSION pHTTPSession = NULL;
...
OAL_U32 HTTPClientSetCredentials (HTTP_SESSION_HANDLE pSession, 
                                 OAL_S8 *pUserName,   // [IN] Null terminated string containing the sessions user name
                                 OAL_S8 *pPassword)   // [IN] Null terminated string containing the sessions password
{
    OAL_U32  nLength;
    P_HTTP_SESSION pHTTPSession = NULL;
...
OAL_U32 HTTPClientAddRequestHeaders (HTTP_SESSION_HANDLE pSession,
                                    OAL_S8 *pHeaderName,      // [IN] The Headers name
                                    OAL_S8 *pHeaderData,      // [IN] The headers data
                                    BOOL nInsert)           // [IN] Reserved could be any
{
    
    OAL_U32  nRetCode;
    OAL_U32  nHeaderLength,nDataLength;
    P_HTTP_SESSION pHTTPSession = NULL;
...
OAL_U32 HTTPClientSendRequest (HTTP_SESSION_HANDLE pSession,
                              OAL_S8 *pUrl,           //  [IN] The requested URL
                              OAL_PTR pData,          //  [IN] Data to post to the server
                              OAL_U32 nDataLength,   //  [IN] Length of posted data
                              BOOL TotalLength,     //  [IN] 
                              OAL_U32 nTimeout,      //  [IN] Operation timeout
							  OAL_U32 nClientPort,   //  [IN] Client side port 0 for none
							  OAL_S8 *Range,
							  OAL_U32 Range_V)       
{
    
    OAL_U32          nRetCode;               // Function call return code
    OAL_U32          nBytes;                 // Bytes counter (socket operations)
    OAL_U32          nUrlLength;             // Length of the given Url
    P_HTTP_SESSION   pHTTPSession = NULL;    // Session pointer
    OAL_S8           ContentLength[32];
    ...
    do
    {
		...
		if (Range_V==TRUE&&strlen(Range)>0)
		{
			if ((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession, "Range", 5, Range, strlen(Range))) != HTTP_CLIENT_SUCCESS)
			{
				break;
			}
		}
...
OAL_U32 HTTPClientRecvResponse (HTTP_SESSION_HANDLE pSession,
                               OAL_U32 nTimeout) // [IN] Timeout for the operation
{
    
    OAL_U32          nRetCode;               // Function return code
    P_HTTP_SESSION  pHTTPSession = NULL;    // Session pointer
...
OAL_U32 HTTPClientWriteData (HTTP_SESSION_HANDLE pSession, 
	                        OAL_PTR pBuffer,
                            OAL_U32 nBufferLength,
                            OAL_U32 nTimeout)
{
    
    OAL_U32          nRetCode     = HTTP_CLIENT_SUCCESS;
    OAL_U32          nBytes;
    OAL_S8            Chunk[HTTP_CLIENT_MAX_CHUNK_HEADER];
...
OAL_U32 HTTPClientReadData(HTTP_SESSION_HANDLE pSession,
	OAL_PTR pBuffer,           // [IN OUT] a pointer to a buffer that will be filled with the servers response
	OAL_U32 nBytesToRead,     // [IN]     The size of the buffer (numbers of bytes to read)
	OAL_U32 nTimeout,         // [IN]     operation timeout in seconds
	OAL_U32 *nBytesRecived)   // [OUT]    Count of the bytes that ware received in this operation
{

	OAL_U32          nBytes = 0;
	OAL_U32          nRetCode = 0;
	OAL_S32           nProjectedBytes = 0; // Should support negative numbers
	...
	nBytes = nBytesToRead; // We will spare 1 byte for the trailing null termination
	*((OAL_S8*)pBuffer) = 0;                // Null terminate the user supplied buffer
	*(nBytesRecived) = 0;                // Set the return bytes count to 0
	...
	if (pHTTPSession->HttpHeadersInfo.nHTTPContentLength > 0)
	{
		...
		if (nProjectedBytes >= (OAL_S32)pHTTPSession->HttpHeadersInfo.nHTTPContentLength)
		{
	...
	nRetCode = HTTPIntrnRecv(pHTTPSession, (OAL_S8*)pBuffer, &nBytes, FALSE);
	// Set the return bytes count
	*(nBytesRecived) = nBytes;   // + 1; Fixed 11/9/2005   
	// Pointer to the end of the buffer
   // pNullPtr    = (OAL_S8*)pBuffer + nBytes ;   
	// And null terminate   
	//*pNullPtr = 0;
...
OAL_U32 HTTPIntrnSetURL (P_HTTP_SESSION pHTTPSession,
                        OAL_S8 *pUrl,         // [IN] a null terminated string containing the Url we should retrieve
                        OAL_U32 nUrlLength)  // [IN] The length the Url string
{
    
    OAL_U32         nUrlOffset;      // Offset in bytes within the Url string
    HTTP_URL       *pUrlPtr;        // a Pointer to the Url structure (within the global session structure)
    OAL_S8           *pPtr;           // a Pointer for the Url port (Used in the parsing process)
    OAL_S8           UrlPort[16];     // a temporary byte array for the Url port conversion operation (string to number)
        ...
        &pUrlPtr->UrlPort) == TRUE)
    {
        if((pUrlPtr->UrlHost.nLength - pUrlPtr->UrlPort.nLength) < 10)
        {
            ...
            while(*pPtr && pPtr++)
            {
                ...
                {
                    *pPtr = 0;
                    pUrlPtr->nPort = (OAL_U16)atol(UrlPort);
...
OAL_U32 HTTPIntrnResizeBuffer (P_HTTP_SESSION pHTTPSession,
                              OAL_U32 nNewBufferSize)    // [IN] The new (and larger) buffer size
{
    
    OAL_S8                *pPtr           = NULL;
    OAL_U32              nCurrentBufferSize;
    ...
    pPtr = (OAL_S8*)OAL_malloc(ALIGN(nNewBufferSize));
    if(!pPtr)
    {
        // malloc() error
        return HTTP_CLIENT_ERROR_NO_MEMORY;
...
OAL_U32 HTTPIntrnHeadersRemove (P_HTTP_SESSION pHTTPSession,
                               OAL_S8 *pHeaderName)   // [IN] The header's name
...
OAL_U32 HTTPIntrnHeadersAdd (P_HTTP_SESSION pHTTPSession,
                            OAL_S8 *pHeaderName,   // [IN] The header's name
                            OAL_U32 nNameLength,  // [IN] Name length
                            OAL_S8 *pHeaderData,   // [IN] The Header's data
                            OAL_U32 nDataLength)  // [IN] Data length
{
    OAL_S8            *pPtr;
    OAL_U32          nProjectedHeaderLength;
    OAL_U32          nProjectedBufferLength;
    OAL_S32           nCurrentfreeSpace;
    OAL_S32           nProjectedfreeSpace;
    OAL_U32          nRetCode;
    ...
    if((OAL_S32)nProjectedfreeSpace < 0)
    {
...
OAL_U32 HTTPIntrnConnectionClose (P_HTTP_SESSION pHTTPSession)
{
    ...
    OAL_S32  nRetCode = HTTP_CLIENT_SUCCESS;
    ...
    do
    {
        ...
        if(pHTTPSession->HttpConnection.HttpSocket != HTTP_INVALID_SOCKET)// INVALID_SOCKET
        {
            ...
            // Gracefully close it
            OAL_socketShutdown(pHTTPSession->HttpConnection.HttpSocket,0x01);
            OAL_socketClose(pHTTPSession->HttpConnection.HttpSocket);
...
{
    OAL_S32            nRetCode = HTTP_CLIENT_SUCCESS;     // a function return code value
    OAL_U32            nNullOffset;                        // a helper value to null terminate a given string
    OAL_S8             Backup;                             // a container for a char value (helps in temporary null termination) 
    ...
    do
    {
        ...
        if(pHTTPSession->HttpConnection.HttpSocket == HTTP_INVALID_SOCKET)
        {
            ...
            pHTTPSession->HttpConnection.HttpSocket = OAL_socket(AF_INET,	    // Address family
        ...
        nRetCode = OAL_socketSetNONBlock(pHTTPSession->HttpConnection.HttpSocket);
        if(nRetCode != 0)
        {
        ...
        */
        ...
        if(pHTTPSession->HttpConnection.HttpClientPort != 0)
        {
            ...
            nRetCode = OAL_socketBind(pHTTPSession->HttpConnection.HttpSocket,
        ...
        {
            nRetCode =OAL_socketConnect(pHTTPSession->HttpConnection.HttpSocket,	// Socket
                (HTTP_SOCKADDR*)&ServerAddress,			                // Server address    
                sizeof(HTTP_SOCKADDR));		                    // Length of server address structure
        }
       //if(nRetCode == 0)
...
OAL_U32 HTTPIntrnSend (P_HTTP_SESSION pHTTPSession,
                      OAL_S8 *pData,            // [IN] a pointer to the data to be sent
                      OAL_U32 *nLength)        // [IN OUT] Length of data to send and the transmitted bytes count
{
    OAL_S32           nSocketEvents;                   // Socket events center
    OAL_S32           nRetCode = HTTP_CLIENT_SUCCESS;   // a function return code value
    HTTP_TIMEVAL    Timeval = { 10 , 0 };            // Timeout value for the socket() method
    ...
    do
    {
        ...
        while(1)
        {
            ...
            nSocketEvents = OAL_socketSelect((pConnection->HttpSocket + 1), 0,
                &pConnection->FDWrite,
                &pConnection->FDError,
                &Timeval);
            ...
            if(FD_ISSET(pConnection->HttpSocket ,&pConnection->FDWrite))
            {
                ...
                {
                    nRetCode = OAL_socketSend(pConnection->HttpSocket,pData,*(nLength),0);
                }
            ...
            {
                OAL_log("HTTPIntrnSend:FD_ISSET");
                FD_CLR((OAL_U32)pConnection->HttpSocket,&pConnection->FDError);
                *(nLength) = 0;
...
OAL_U32 HTTPIntrnRecv (P_HTTP_SESSION pHTTPSession,
                      OAL_S8 *pData,        // [IN] a pointer for a buffer that receives the data
                      OAL_U32 *nLength,    // [IN OUT] Length of the buffer and the count of the received bytes
                      BOOL PeekOnly)      // [IN] State if we should only peek the socket (default is no)
{
    OAL_S32      nSocketEvents;
    OAL_S32      nRetCode = HTTP_CLIENT_SUCCESS;
    HTTP_TIMEVAL Timeval = { 10, 0};
    ...
    do
    {
        ...
        while(1)
        {
            ...
            // See if we got any events on the socket
            nSocketEvents = OAL_socketSelect(pConnection->HttpSocket + 1, &pConnection->FDRead,
                0,
                &pConnection->FDError,
                &Timeval);
            ...
            if(nSocketEvents == 0)
            {
                OAL_log("recv select nSocketEvents =0");
                ///////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if(FD_ISSET(pConnection->HttpSocket ,&pConnection->FDRead)) // Are there any read events on the socket ?
            {
                //OAL_log("recv select nSocketEvents >0");
                // Clear the event 
                FD_CLR((OAL_U32)pConnection->HttpSocket,&pConnection->FDRead);
                ...
                {     
                    ...
                    {
                        if((nRetCode = OAL_socketRecv(pConnection->HttpSocket,pData,*(nLength),0)) == SOCKET_ERROR)
                        {
                            // Socket error
                            nRetCode =  HTTP_CLIENT_ERROR_SOCKET_RECV;
                            break;
                ...
                {
                    // Only peek te socket
                    if((nRetCode = OAL_socketRecv(pConnection->HttpSocket,pData,*(nLength),MSG_PEEK)) == SOCKET_ERROR)
                    {
                        // Socket error
                        nRetCode =  HTTP_CLIENT_ERROR_SOCKET_RECV;
                        break;
            ...
            if(FD_ISSET(pConnection->HttpSocket ,&pConnection->FDError))
            {
                ...
                *(nLength) = 0;
                ...
                // To-Do: Handle this case
                nRetCode = HTTP_CLIENT_ERROR_SOCKET_RECV;
                OAL_log("HTTP_CLIENT_ERROR_SOCKET_RECV:FD_ISSET");
                break;
...
OAL_U32 HTTPIntrnGetRemoteChunkLength (P_HTTP_SESSION pHTTPSession)
{
    ...
    OAL_U32          nBytesRead = 1;
    OAL_U32          nRetCode = HTTP_CLIENT_SUCCESS;
    OAL_U32          nBytesCount = 0;
    OAL_S8            ChunkHeader[HTTP_CLIENT_MAX_CHUNK_HEADER];
    OAL_S8            *pPtr;
    ...
    do
    {
        ...
        while(nBytesRead > 0)
        {
            ...
            if(nRetCode == HTTP_CLIENT_SUCCESS && nBytesRead > 0)
            {
                ...
                pHTTPSession->HttpHeaders.HeadersIn.nLength++;
                //pHTTPSession->HttpHeaders.HeadersIn.nLength+= nBytesRead;
                // Set the HTTP counters
                pHTTPSession->HttpCounters.nRecivedHeaderLength++;
                //pHTTPSession->HttpCounters.nRecivedHeaderLength+= nBytesRead;
...
OAL_U32 HTTPIntrnHeadersFind (P_HTTP_SESSION pHTTPSession,OAL_S8 *pHeaderName,
                             ...
                             OAL_U32 nOffset)         //  [IN]  Optionaly privide an offset to start looking from
{
    OAL_S8           *pHeaderEnd;
    OAL_S8           Header[HTTP_CLIENT_MAX_HEADER_SEARCH_CLUE]; // To-Do: Use pointers insted of fixed length
    OAL_U32         nLength;
    OAL_U32         nRetCode = HTTP_CLIENT_ERROR_HEADER_NOT_FOUND;
...
{
    OAL_U32      nRetCode = HTTP_CLIENT_SUCCESS;   // Function call return code
    OAL_U32      nBytes = 32;
    OAL_U32      nTotalBytes = 0;
    OAL_S8        ErrorPage[32];
...
OAL_U32 HTTPIntrnHeadersParse (P_HTTP_SESSION pHTTPSession)
{
    ...
    OAL_S8            *pPtr;                                      // a pointer that points on the incoming headers
    OAL_U32          nTokenLength = 0;                           // Length of the parsed token
    OAL_U32          nRetCode = HTTP_CLIENT_SUCCESS;             // a function return code value
    OAL_U32          nOffset = 0;                                // Bytes offset (strings comperision)
    OAL_S8            HTTPToken[HTTP_CLIENT_MAX_TOKEN_LENGTH];    // Buffer for the parsed HTTP token
    ...
    do
    {
        ...
        strncpy(HTTPToken,(pPtr - nTokenLength),MINIMUM(HTTP_CLIENT_MAX_TOKEN_LENGTH,nTokenLength));
...
OAL_U32 HTTPIntrnParseAuthHeader(P_HTTP_SESSION pHTTPSession)
{
    ...
    if(pHTTPSession->HttpAuthHeader.HTTP_AUTH_SCHEMA != (OAL_U32)pHTTPSession->HttpCredentials.CredAuthSchema)
    {
...
OAL_U32 HTTPIntrnHeadersSend(P_HTTP_SESSION pHTTPSession,
                            ...
                            // Can be used for evaluating a HEAD request
{
    ...
    if(pHTTPSession->pDebug)
    {
        pHTTPSession->pDebug("HTTPIntrnHeadersSend",NULL,
            0,"Using Verb: %d",(OAL_S32)HttpVerb);
    }
#endif
...
OAL_U32 HTTPIntrnAuthSendBasic (P_HTTP_SESSION pHTTPSession)
{
    ...
    OAL_U32      nSegmentLength;
    OAL_U32      nRetCode;
    OAL_S8        Cred[HTTP_CLIENT_MAX_64_ENCODED_CRED /2];   // Credentials (Clear) 
    OAL_S8        Cred64[HTTP_CLIENT_MAX_64_ENCODED_CRED];    // Credentials (64 bit encoded)
    OAL_U32      nSrcLength, nDestLength;
    OAL_S8*       pPtr;
    OAL_S8*       INITIAL_HDR         = "Authorization: Basic ";
    OAL_S8*       INITIAL_PROXY_HDR   = "Proxy-Authorization: Basic ";
...
OAL_U32 HTTPIntrnAuthSendDigest (P_HTTP_SESSION pHTTPSession)
{
    ...
    OAL_U32      nSegmentLength;
    OAL_U32      nRetCode;
    OAL_U32      nAlgType = 0; // a flag for the algorithem type (default to MD5)
    ...
    OAL_S8*       INITIAL_HDR         = "Authorization: Digest username=\"";
    OAL_S8*       INITIAL_PROXY_HDR   = "Proxy-Authorization: Digest username=\"";
    OAL_S8*       REALEM_HDR          = "\", realm=\"";
    OAL_S8*       QOP_HDR             = "\", qop=\"";
    OAL_S8*       ALGO_HDR            = "\", algorithm=\"";
    OAL_S8*       URI_HDR             = "\", uri=\"";
    OAL_S8*       NONCE_HDR           = "\", nonce=\"";
    OAL_S8*       NC_HDR              = "\", nc=00000001, cnonce=\""; // To-Do: This should be tested!!
    OAL_S8*       RSP_HDR             = "\", response=\"";
    OAL_S8*       OPQ_HDR             = "\", opaque=\"";
...
{
    OAL_U32 nActionTimeout; // For restoring a parameter after this reset
    OAL_U32 nAllocationSize;
...
OAL_U32 HTTPIntrnHeadersReceive (P_HTTP_SESSION pHTTPSession,
                                OAL_U32 nTimeout)        // [IN] Timeout for the operation
...
BOOL HTTPIntrnSessionEvalTimeout(P_HTTP_SESSION pHTTPSession)
{
	...
	return FALSE;
#else
    OAL_U32 nElapsedTime;    // Integer for calculating the elapsed time
