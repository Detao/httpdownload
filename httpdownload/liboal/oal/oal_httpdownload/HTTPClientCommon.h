...
#define HTTP_CLIENT_EOS                     1000        // HTTP end of stream message
    ...
    {
        OAL_U32        HTTPStatusCode;                 // HTTP Status code (200 OK)
        OAL_U32		    RequestBodyLengthSent;          // Total bytes sent (body only)
        OAL_U32		    ResponseBodyLengthReceived;     // Total bytes received (body only)
        OAL_U32		    TotalResponseBodyLength;        // as extracted from the "ontent-length" header
        OAL_U32        HttpState;
    } HTTP_CLIENT;
